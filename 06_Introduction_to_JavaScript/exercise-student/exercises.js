...
*/
		function hasTeen(x, y, z) {
			let firstRange = x >= 13 && x <= 19;
			let secondRange = y >= 13 && y <= 19;
			let thirdRange = z >= 13 && z <= 19;
...
*/
			...
			function cigarParty(num, isWeekend = true) {
				let numOfCigar = num >= 40 && num <= 60;
				let result = false;
				if(numOfCigar && !isWeekend) {
					result = true;
				}
				if(!numOfCigar && !isWeekend) {
					return result;
...
*/
		...
		function filterMultiplesOfX(arr, num) {
			return arr.filter(number => number % num === 0);
		}
